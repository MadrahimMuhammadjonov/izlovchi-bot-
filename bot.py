...
# Userbot: in_memory=True Railway-da sessiya fayli yaratmaslik uchun
userbot = Client(
    "my_userbot",
    api_id=API_ID,
    api_hash=API_HASH,
...
async def sub_menu(c: types.CallbackQuery):
    ...
    await c.message.edit_text("Amalni tanlang:", reply_markup=kb.as_markup())
...
async def start_add(c: types.CallbackQuery):
    ...
    txt = "📝 So'zlarni vergul bilan yuboring:" if mode == "adding_kw" else "📡 Guruh username yoki havolasini yuboring:"
