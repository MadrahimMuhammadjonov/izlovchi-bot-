...
PERSONAL_GROUP_ID = -1003267783623 # Bu ID ni yana bir bor tekshiring
...
async def message_watcher(client, message):
    try:
        ...
        if found_words:
            # Siz so'ragan aniq format
            user = message.from_user
            u_id = user.id if user else "noma'lum"
            u_name = html.escape(user.full_name if user else "Noma'lum foydalanuvchi")
...
async def start_adding(c: types.CallbackQuery):
    ...
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🔙 Orqaga", callback_data="menu_kw" if mode == "adding_kw" else "menu_gr")
    ]])
    
    txt = "📝 Kalit so'zlarni yuboring (vergul bilan):" if mode == "adding_kw" else "📡 Guruh havolasini yoki @username yuboring:"
...
async def handle_admin_input(m: types.Message):
    ...
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Orqaga", callback_data=back_target)]])
    ...
    if state == "adding_kw":
        ...
        await m.answer(f"✅ {len(words)} ta so'z qo'shildi. Yana yuboring yoki Orqaga tugmasini bosing:", reply_markup=back_kb)
        ...
        try:
            ...
            await m.answer(f"✅ Guruh qo'shildi: <b>{chat.title}</b>\nNavbatdagisini yuboring yoki Orqaga tugmasini bosing:", reply_markup=back_kb, parse_mode="HTML")
        except Exception as e:
            await m.answer(f"❌ Xato: {e}\nQayta urinib ko'ring:", reply_markup=back_kb)
