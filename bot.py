...
# --- DB OPERATSIYALARI (Asinxron wrapper) ---
...
async def init_db():
    await db_op('CREATE TABLE IF NOT EXISTS keywords (id INTEGER PRIMARY KEY, keyword TEXT UNIQUE)')
    await db_op('CREATE TABLE IF NOT EXISTS search_groups (id INTEGER PRIMARY KEY, group_id INTEGER UNIQUE, group_name TEXT)')
    await db_op('CREATE TABLE IF NOT EXISTS user_state (user_id INTEGER PRIMARY KEY, state TEXT, data TEXT)')

# --- XABARLARNI KUZATISH (Asinxron yuborish) ---
@app.on_message(filters.group)
async def watcher(client, message):
    if not message.text: return
    
    # Guruhlarni tekshirish (Keshdan foydalanish tavsiya etiladi, lekin hozir bazadan)
    res = await db_op("SELECT group_id FROM search_groups", fetch=True)
    groups = [g[0] for g in res]
    if message.chat.id not in groups: return
    
    # Kalit so'zlar
    res_kw = await db_op("SELECT keyword FROM keywords", fetch=True)
    words = [k[0] for k in res_kw]
...
def main_kb():
    ...
    builder.row(InlineKeyboardButton(text="🔑 Kalit so'zlar", callback_data='open_keywords'))
    builder.row(InlineKeyboardButton(text="📡 Izlovchi guruhlar", callback_data='open_groups'))
...
async def start(m: types.Message):
    if m.from_user.id in ADMIN_LIST:
        await db_op("DELETE FROM user_state WHERE user_id=?", (m.from_user.id,))
...
async def go_home(c: types.CallbackQuery):
    await db_op("DELETE FROM user_state WHERE user_id=?", (c.from_user.id,))
...
async def add_start(c: types.CallbackQuery):
    ...
    await db_op("REPLACE INTO user_state VALUES (?, ?, ?)", (c.from_user.id, mode, ""))
    
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Orqaga", callback_data='main_home')]])
    txt = "📝 So'zlarni yuboring (vergul bilan):" if mode == "adding_kw" else "📡 Guruh @username yoki linkini yuboring:"
    await c.message.edit_text(txt, reply_markup=back_kb)

@dp.message(F.text)
async def handle_text(m: types.Message):
    if m.from_user.id not in ADMIN_LIST: return
    res = await db_op("SELECT state FROM user_state WHERE user_id=?", (m.from_user.id,), fetch=True)
    if not res: return
    
    state = res[0][0]
    if state == "adding_kw":
        words = [w.strip() for w in m.text.split(",") if w.strip()]
        for w in words: await db_op("INSERT OR IGNORE INTO keywords (keyword) VALUES (?)", (w,))
        await m.answer(f"✅ {len(words)} ta so'z qo'shildi. Yana yuboring yoki 'Orqaga' bosing.")
    
    elif state == "adding_gr":
        link = m.text.strip().replace("@", "").replace("https://t.me/", "").split("/")[0]
        try:
            chat = await app.get_chat(link)
            await db_op("INSERT OR IGNORE INTO search_groups (group_id, group_name) VALUES (?, ?)", (chat.id, chat.title))
            await m.answer(f"✅ Qo'shildi: <b>{chat.title}</b>\nNavbatdagisini yuboring...")
        except Exception as e:
...
async def view_list(c: types.CallbackQuery):
    ...
    data = await db_op(f"SELECT {col} FROM {table}", fetch=True)
...
async def del_list(c: types.CallbackQuery):
    ...
    table = "keywords" if mode == "kw" else "search_groups"
    col = "keyword" if mode == "kw" else "group_name"
    data = await db_op(f"SELECT id, {col} FROM {table}", fetch=True)
    
    builder = InlineKeyboardBuilder()
    for x in data:
        builder.row(InlineKeyboardButton(text=f"🗑 {x[1]}", callback_data=f"rm_{mode}_{x[0]}"))
    builder.row(InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"open_{'keywords' if mode=='kw' else 'groups'}"))
...
async def remove_item(c: types.CallbackQuery):
    ...
    table = "keywords" if mode == "kw" else "search_groups"
    await db_op(f"DELETE FROM {table} WHERE id=?", (item_id,))
    await c.answer("O'chirildi!")
    await view_list(c)

@dp.callback_query(F.data == "sys_status")
async def status(c: types.CallbackQuery):
    k = (await db_op("SELECT COUNT(*) FROM keywords", fetch=True))[0][0]
    g = (await db_op("SELECT COUNT(*) FROM search_groups", fetch=True)[0][0] )
    txt = f"⚙️ <b>Status:</b>\n\n🔑 So'zlar: {k}\n📡 Guruhlar: {g}\n✅ Bot va Userbot: Faol"
